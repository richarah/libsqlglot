...
| **Codebase** | 1,415,254 lines C++ | 50,000+ lines Python |
...
**Code quality**: Compiles with `-Wall -Wextra -Wpedantic -Werror`. No runtime dependencies. No RTTI. Passes 27,043 assertions across 360 test cases. Fuzz-tested with `libFuzzer` + `AddressSanitizer`.
...
1,415,254 lines of C++ headers, 18 files, no `.cpp`. See `include/libsqlglot/` for the full layout. The big ones: `parser.h` (2890 lines), `generator.h` (1639), `expression.h` (1100, 100 expression types). Entry point is `transpiler.h` (86 lines).
...
360 test cases, 27,043 assertions, all passing.
